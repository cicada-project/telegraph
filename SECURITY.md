...
The R4 review covers pairing, identity verification, device-code binding, human safety-number or fingerprint confirmation, key lifecycle, relay and retention, offline delivery, message ordering, replay protection, key rotation, revocation, and audit privacy. Its design-review conditions and historical gate-local metadata remain open and unchanged. The subsequent T2/T3 implementation addendum extends only the independently reviewed ownership, dependency, storage, and test conditions; T4/T5/T6+ and release/deployment remain blocked. No implementation result may be treated as E2EE or production evidence.
